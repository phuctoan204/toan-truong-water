...
namespace WebBanHangOnline.Controllers
{
    ...
    public class ShoppingCartController : Controller
    {
        ...
        //    var discountAmount = Session["DiscountAmount"] != null ? (decimal)Session["DiscountAmount"] : 0;
        //    if (cart != null)
        //    {
        //        // Trừ giảm giá nếu có
        //        ViewBag.TotalAfterDiscount = cart.Items.Sum(x => x.TotalPrice) - discountAmount;
        //        return PartialView(cart.Items);
        //    }
        ...
        public ActionResult CheckOut(OrderViewModel req)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (cart != null)
                {
                    ...
                    decimal total = cart.Items.Sum(x => x.Price * x.Quantity);
                    decimal discountAmount = Session["DiscountAmount"] != null ? (decimal)Session["DiscountAmount"] : 0;
                    ...
                    order.DiscountCode = Session["DiscountCode"] != null ? Session["DiscountCode"].ToString() : null;
                    ...
                    db.Orders.Add(order);
                    db.SaveChanges();
                    // ✅ Cập nhật mã giảm giá đã được sử dụng
                    if (Session["DiscountCode"] != null)
                    {
                        string discountCode = Session["DiscountCode"].ToString();
                        var codeUsed = db.DiscountCodes.FirstOrDefault(x => x.Code == discountCode);
                        if (codeUsed != null)
                        {
                            codeUsed.IsUsed = true;
                            db.Entry(codeUsed).State = System.Data.Entity.EntityState.Modified;
                            db.SaveChanges(); // Lưu lại cập nhật trạng thái
                        }
                    }
                    ...
                    //TongTien = thanhtien;
                    var thanhtien = decimal.Zero;
                    discountAmount = Session["DiscountAmount"] != null ? (decimal)Session["DiscountAmount"] : 0;
                    ...
                    foreach (var sp in cart.Items)
                    {
                        ...
                        strSanPham += "<td>" + WebBanHangOnline.Common.Common.FormatNumber(sp.TotalPrice, 0) + "</td>";
                        strSanPham += "</tr>";
                        thanhtien += sp.TotalPrice; // ✅ Dùng giá đã nhân số lượng (sau khi giảm nếu có)
                    }

                    decimal tongSauGiam = thanhtien - discountAmount;
                    ...
                    contentCustomer = contentCustomer.Replace("{{GiamGia}}", WebBanHangOnline.Common.Common.FormatNumber(discountAmount, 0));
                    contentCustomer = contentCustomer.Replace("{{TongTien}}", WebBanHangOnline.Common.Common.FormatNumber(tongSauGiam, 0));
                    ...
                    contentAdmin = contentAdmin.Replace("{{GiamGia}}", WebBanHangOnline.Common.Common.FormatNumber(discountAmount, 0));
                    contentAdmin = contentAdmin.Replace("{{TongTien}}", WebBanHangOnline.Common.Common.FormatNumber(tongSauGiam, 0));
        ...
        public ActionResult AddToCart(int id, int quantity)
        {
            ...
            if (checkProduct != null)
            {
                ...
                Session["Cart"] = cart;
                var currentCart = Session["Cart"] as ShoppingCart;
                code = new { Success = true, msg = "Thêm sản phẩm vào giỏ hàng thành công!", code = 1, Count = cart.Items.Count };

                //

                //string userEmail = Session["UserEmail"]?.ToString(); // hoặc lấy từ Identity

                string userEmail = "phuctoan351@gmail.com";
                Console.WriteLine("send email");
                if (cart != null && !string.IsNullOrEmpty(userEmail))
                {
                    Task.Run(async () =>
                    {
                        await Task.Delay(2000); // Chờ 1 phút
                                                // Kiểm tra lại session


                        // Nếu sau 60 giây giỏ hàng vẫn còn -> chưa thanh toán
                        if (currentCart != null)
                        {
                            var emailService = new SendEmailService();
                            //thử đổi usẻ email thành 1 email hard code
            ...
            // Giả sử bạn lấy email của người dùng từ Session
            //string userEmail = Session["UserEmail"]?.ToString();

            //if (!string.IsNullOrEmpty(userEmail))
            //{
            //    // Gửi email sau 1 phút
            //    Task.Run(async () =>
            //    {
            //        await Task.Delay(60000); // Đợi 1 phút (60,000ms)
            //        string subject = "Mua ngay kẻo hết deal hời";
            //        string body = $"<h3>Chào bạn,</h3><p>Bạn vừa thêm <b>{product.Title}</b> vào giỏ hàng. Hãy mua ngay trước khi hết deal nhé!</p>";
        ...
        public JsonResult ApplyDiscount(string discountCode)
        {
            ...
            {
                var discount = db.DiscountCodes.FirstOrDefault(d =>
                    d.Code == discountCode &&
                ...
                decimal discountAmount = (total * (decimal)discount.DiscountPercent) / 100;
                ...
                decimal newTotal = total - discountAmount;

                Session["DiscountAmount"] = discountAmount;
                Session["DiscountCode"] = discountCode;

                return Json(new { success = true, discountAmount = discountAmount, newTotal = newTotal });
            }
        }
